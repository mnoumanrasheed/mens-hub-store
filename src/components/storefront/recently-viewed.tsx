...
export function RecentlyViewed({ current }: { current: Item }) {
  ...
  return <section className="mt-20 border-t border-line pt-14"><p className="store-eyebrow">Your edit</p><h2 className="font-display text-4xl text-ivory">Recently viewed</h2><div className="mt-7 grid grid-cols-1 gap-6 min-[430px]:grid-cols-2 min-[430px]:gap-3 sm:grid-cols-4">{items.map((item) => <ProductTrackedLink productId={item.id} key={item.id} href={`/product/${item.slug}`} className="group"><span className="relative block aspect-[4/5] overflow-hidden bg-surface"><StoreImage src={item.imageUrl} alt={item.name} fill sizes="(max-width: 429px) 100vw, (max-width: 639px) 50vw, 25vw" className="object-cover transition group-hover:scale-[1.02]" /></span><span className="mt-3 block truncate font-display text-xl text-ivory">{item.name}</span><span className="text-sm text-muted">Article {item.sku} · PKR {item.price}</span></ProductTrackedLink>)}</div></section>;
}
